...
- **computer_exec_tail** — ADR-0005 in-flight output: the `CheckOutput` 5 s self-loop reads `/tmp/.exec-out/<exec_id>.log` mid-run and reports `ReportOutput(stdout_tail, stdout_bytes)` so the Exec row (and any `/tdata/$events` subscriber) shows live output while the command is still Running. Never errors: any failure degrades to a stay (`wasm/computer_exec_tail/`).
